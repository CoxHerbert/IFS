...
func init() {
	articleDaoImpl = &articleDao{
		selectSql: `select article_id, title, slug, summary, category, cover_url, content, status, sort,
			ifnull(concat(date_format(publish_time, '%Y-%m-%d %H'), char(58),
				date_format(publish_time, '%i'), char(58), date_format(publish_time, '%s')), '') as publish_time,
