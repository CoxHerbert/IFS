...
type PaymentDeclarationReviewReq struct {
	Remark string `json:"remark"`
}

type PaymentLedgerDQL struct {
	ShipmentNo    string `form:"shipmentNo" db:"shipment_no"`
	CustomerId    int64  `form:"customerId" db:"customer_id"`
	CustomerName  string `form:"customerName" db:"customer_name"`
	PaymentStatus string `form:"paymentStatus" db:"payment_status"`
	Currency      string `form:"currency" db:"currency"`
	SalesUserId   int64  `form:"salesUserId" db:"sales_user_id"`
	commonModels.BaseEntityDQL
}

type PaymentLedgerVo struct {
	ShipmentId    int64   `json:"shipmentId,string" db:"shipment_id"`
	ShipmentNo    string  `json:"shipmentNo" db:"shipment_no"`
	...
	UnpaidAmount  float64 `json:"unpaidAmount" db:"unpaid_amount"`
	PaymentStatus string  `json:"paymentStatus" db:"payment_status"`
	Currency      string  `json:"currency" db:"currency"`
}

type PayableAmountUpdateReq struct {
	PayableAmount float64 `json:"payableAmount"`
}

type ShipmentChargeItem struct {
	ChargeId   int64   `json:"chargeId,string" db:"charge_id"`
	ShipmentId int64   `json:"shipmentId,string" db:"shipment_id"`
	FeeName    string  `json:"feeName" db:"fee_name"`
	Amount     float64 `json:"amount" db:"amount"`
	Currency   string  `json:"currency" db:"currency"`
	Remark     string  `json:"remark" db:"remark"`
}

type ShipmentChargeUpdateReq struct {
	Items []*ShipmentChargeItem `json:"items"`
}
