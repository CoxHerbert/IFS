  ...
  <div class="app-container receipt-page">
    ...
    <vxe-modal v-model="createOpen" title="新增收款" width="760" show-footer>
      <vxe-form :data="form" title-width="90">
        ...
        <vxe-form-item title="币种" span="12" :item-render="{}"><template #default><vxe-select v-model="form.currency" filterable @change="currencyChanged"><vxe-option v-for="item in currencyOptions" :key="item.value" :value="item.value" :label="item.label" /></vxe-select></template></vxe-form-item>
      ...
      <vxe-table border :data="form.allocations">
        <vxe-column title="出货计划" min-width="260"><template #default="{row}"><vxe-select v-model="row.shipmentId" filterable @change="shipmentChanged(row, $event)"><vxe-option v-for="s in shipments" :key="s.shipmentId" :value="s.shipmentId" :label="s.shipmentNo+' / '+(s.orderNo||'-')+' / 未收 '+money(s.unpaidAmount)" /></vxe-select></template></vxe-column>
        <vxe-column title="应收金额" width="145" align="right"><template #default="{row}">{{ form.currency }} {{ money(row.payableAmount) }}</template></vxe-column>
        <vxe-column title="已收金额" width="145" align="right"><template #default="{row}">{{ form.currency }} {{ money(row.paidAmount) }}</template></vxe-column>
        <vxe-column title="未收金额" width="145" align="right"><template #default="{row}">{{ form.currency }} {{ money(row.unpaidAmount) }}</template></vxe-column>
        <vxe-column title="核销金额" width="210"><template #default="{row}"><div class="money-input"><span>{{ form.currency }}</span><vxe-number-input v-model="row.allocatedAmount" type="float" min="0" :digits="2" /></div></template></vxe-column>
    ...
    <vxe-modal v-model="detailOpen" title="收款详情" width="700" :show-footer="false">
      <div v-if="detail" class="detail-grid"><div>收款单号：{{detail.receiptNo}}</div><div>客户：{{detail.customerName}}</div><div>金额：{{detail.currency}} {{money(detail.amount)}}</div><div>已核销：{{detail.currency}} {{money(detail.allocatedAmount)}}</div></div>
      <vxe-table border :data="detail?.allocations||[]"><vxe-column field="shipmentNo" title="出货计划"/><vxe-column title="核销金额"><template #default="{row}">{{ detail?.currency }} {{ money(row.allocatedAmount) }}</template></vxe-column></vxe-table>
    </vxe-modal>
  </div>
</template>
...
import { listPaymentLedger } from '@/api/freight/payment'
...
function openCreate(){Object.assign(form,{customerId:'',customerName:'',amount:0,currency:'CNY',receiptTime:'',paymentMethod:'BANK_TRANSFER',remark:'',allocations:[{shipmentId:'',allocatedAmount:0}]});files.value=[];shipments.value=[];createOpen.value=true;simpleCustomerOptions({}).then(r=>customers.value=r.data||[])}
function emptyAllocation(){return {shipmentId:'',allocatedAmount:0,payableAmount:0,paidAmount:0,unpaidAmount:0}}
function loadShipmentOptions(){form.allocations=[emptyAllocation()];if(!form.customerId){shipments.value=[];return}listPaymentLedger({pageNum:1,pageSize:100,customerId:form.customerId,currency:form.currency}).then(r=>shipments.value=r.data?.rows||[])}
function customerChanged({value}){const c=customers.value.find(x=>x.customerId===value);form.customerName=c?.customerName||'';loadShipmentOptions()}
function currencyChanged(){loadShipmentOptions()}
function addAllocation(){form.allocations.push(emptyAllocation())}
function shipmentChanged(row,{value}){const shipment=shipments.value.find(x=>String(x.shipmentId)===String(value));if(!shipment){Object.assign(row,emptyAllocation());return}const allocatedByOtherRows=form.allocations.filter(x=>x!==row).reduce((sum,item)=>sum+Number(item.allocatedAmount||0),0);const receiptRemaining=Math.max(Number(form.amount||0)-allocatedByOtherRows,0);Object.assign(row,{shipmentId:value,payableAmount:Number(shipment.payableAmount||0),paidAmount:Number(shipment.paidAmount||0),unpaidAmount:Number(shipment.unpaidAmount||0),allocatedAmount:Math.min(Number(shipment.unpaidAmount||0),receiptRemaining)})}
...
<style scoped>.add-button{margin:10px 0}.receipt-page :deep(.vxe-pager){margin-top:12px}.allocation-title{display:flex;justify-content:space-between;align-items:center;margin:12px 0}.hint{color:#909399;font-size:12px}.detail-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px}.money-input{display:flex;align-items:center;gap:6px}.money-input .vxe-number-input{flex:1}</style>
